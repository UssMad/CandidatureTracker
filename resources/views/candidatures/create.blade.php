...
@section('title', 'Nouvelle Candidature - CandidatureTracker')

@section('content')
<div class="max-w-2xl mx-auto w-full">
    <div class="mb-stack-lg">
        <a href="{{ route('candidatures.index') }}" class="inline-flex items-center gap-1 text-on-surface-variant hover:text-primary text-body-sm mb-4 transition-colors">
            <span class="material-symbols-outlined text-sm">arrow_back</span>
            Retour à la liste
        </a>
        <h1 class="text-headline-lg-mobile md:text-headline-lg font-bold text-on-surface">Nouvelle candidature</h1>
        <p class="text-body-sm text-on-surface-variant mt-2">Renseignez les détails de votre candidature pour un suivi optimal.</p>
    </div>

    <div class="bg-surface-container-lowest rounded-xl p-6 md:p-8 soft-shadow">
        <form method="POST" action="{{ route('candidatures.store') }}" class="space-y-6" enctype="multipart/form-data">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                <div class="md:col-span-1">
                    <x-input-label for="entreprise" value="Entreprise *" />
                    <x-text-input id="entreprise" name="entreprise" type="text" class="mt-1 block w-full" :value="old('entreprise')" placeholder="Ex: Google, Acme Corp" />
                    <x-input-error :messages="$errors->get('entreprise')" class="mt-1" />
                </div>

                <div class="md:col-span-1">
                    <x-input-label for="poste" value="Poste visé *" />
                    <x-text-input id="poste" name="poste" type="text" class="mt-1 block w-full" :value="old('poste')" placeholder="Ex: Développeur Full-Stack" />
                    <x-input-error :messages="$errors->get('poste')" class="mt-1" />
                </div>

                <div class="md:col-span-2">
                    <x-input-label for="url_offre" value="URL de l'offre" />
                    <x-text-input id="url_offre" name="url_offre" type="url" class="mt-1 block w-full" :value="old('url_offre')" placeholder="https://..." />
                ...
                <div class="md:col-span-1">
                    ...
                    <select id="statut" name="statut" class="mt-1 block w-full border-outline-variant rounded-lg text-body-sm text-on-surface focus:border-primary focus:ring-primary-container shadow-sm bg-surface-container-lowest">
                        <option value="envoyée" @selected(old('statut', 'envoyée') === 'envoyée')>Envoyée</option>
                        <option value="en_cours" @selected(old('statut') === 'en_cours')>En cours</option>
                        <option value="entretien" @selected(old('statut') === 'entretien')>Entretien</option>
                        <option value="offre" @selected(old('statut') === 'offre')>Offre reçue</option>
                        <option value="refusée" @selected(old('statut') === 'refusée')>Refusée</option>
                        <option value="abandonnée" @selected(old('statut') === 'abandonnée')>Abandonnée</option>
                    </select>
                ...
                <div class="md:col-span-1">
                    ...
                    <select id="priorite" name="priorite" class="mt-1 block w-full border-outline-variant rounded-lg text-body-sm text-on-surface focus:border-primary focus:ring-primary-container shadow-sm bg-surface-container-lowest">
                        ...
                        <option value="faible" @selected(old('priorite') === 'faible')>Faible</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <x-text-input id="date_candidature" name="date_candidature" type="date" class="mt-1 block w-full" :value="old('date_candidature', date('Y-m-d'))" />
                    <x-input-error :messages="$errors->get('date_candidature')" class="mt-1" />
                </div>

                <div class="md:col-span-2">
                    <x-input-label for="notes" value="Notes" />
                    <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full border-outline-variant rounded-lg text-body-sm text-on-surface focus:border-primary focus:ring-primary-container shadow-sm bg-surface-container-lowest resize-y" placeholder="Remarques, contacts, détails importants...">{{ old('notes') }}</textarea>
            ...
            <div class="pt-6 mt-6 border-t border-surface-variant flex items-center justify-end gap-4">
                <a href="{{ route('candidatures.index') }}" class="px-6 py-2 rounded-lg text-body-sm font-medium text-primary border border-transparent hover:bg-surface-container-low focus:outline-none focus:ring-2 focus:ring-primary-container transition-colors">
                    Annuler
                </a>
